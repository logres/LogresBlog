...
![Happy](./assets/Picture/FlagRegister.png)
...
![Comp_Test](./assets/Picture/CMP_TEST.png)
...
![search](./assets/Picture/SearchAddress.png)
...
![condition](./assets/Picture/Con_Transfer.png)
...
![Picture](./assets/Picture/ConMov.png)
...
![](./assets/Picture/3/Y86_ISandCode.jpg)
...
![](./assets/Picture/3/function_code.png)
...
![](./assets/Picture/3/PIPE_Hard.png)
...
![](./assets/Picture/4/KeyPath.png)

#### 4.2.4 并行实现

考虑源程序数据流：

![](./assets/Picture/4/oringin.png)
...
![](./assets/Picture/5/Adress_Code.jpg)
...
下图为Core i7 Haswell架构的处理器的表现。

![](./assets/Picture/5/MM.jpg)
...
![](./assets/Picture/6/ELF.png)

图中可大体了解其结构，接下来我们将逐项分析：

#### 6.1.1 ELF表头

![](./assets/Picture/6/ELFhead.png)
...
多个可重定位文件存在许多重名段，其内容相近，权限亦相近，在映射到内存时该如何处理呢？是按文件存放吗？那样将产生许多小段，占用大量的虚拟页，而且地址上的离散也将带来不便，那么该怎么安排呢？；另一方面，在保证被引用符号的存在性与唯一性之后，链接器需要对所有引用进行修正，保证其能够访问到所引用的数据。那么改哪里？怎么改呢？

#### 6.3.1 重定位节与符号定义

在经历符号解析后，链接器统合所有文件中相似的节，将其放置在一起，成为**聚合节**（section），而权限相近的section则放在一起，成为segment，用于转载和映射。这就是重定位节的过程。此后，每一条指令和全局变量便都有了唯一的运行时地址了。

#### 6.3.2 重定位符号引用

##### 6.3.2.1 重定位表

在介绍ELF文件格式的时候，我们提到过，存在着一些名为重定位表的段，含有重定位条目，指导重定位工作。

![](./assets/Picture/6/REL.png)
...
A：被修正位置的原值
P：被修正位置的地址（从修正的部分开始算，并非被修正的指令地址）
S：被指向符号的实际地址
...
链接器**从左到右**扫描包括的静态链接库与目标模块文件，进行以下环节：
...
这个过程使得链接器能够挑选出需要链接的模块，但同时也使得链接时静态库的**顺序**变得重要。若是先出现符号定义，在出现引用，将导致处理定义的文件被丢弃，后续出现的引用文件找不到定义而报错。
...
因此：仅需要采用相对寻址引用与将绝对地址增加基址的绝对寻址，便可获得正确的引用地址。
...
其缘由在于需要处理 **可执行文件引用共享库模块中全局变量时，其编译器无法分辨该变量是不是共享库中内容，便会在.bss产生副本**（可执行文件不一定做了地址无关处理）的情况。
...
1. 若绑定了，跳转至PLT，修改GOT内容，指向正确引用地址
2. 否则，跳转至PLT，引用GOT内容，指向绑定用代码
...
前文实现了加载时链接中的加载时重定位、位置无关代码、延迟绑定技术，实际上还存在运行时链接技术（无需在静态连接时将动态符号写入elf文件）：
...
**中断**，是**异常控制流**的一种形式，由硬件与操作系统协力实现。

异常有多种来源：

![](./assets/Picture/7/异常类型.png)

1. 硬件中断-**硬中断**:**硬中断**，这在嵌入式开发中最为常见。其往往为外部I/O端口的变化所引起，使得处理器进入异常处理。其不为任何指令所引起，故可能发生在任何时刻，这就导致其相对于其他异常是**异步**的。==（需要主义：由于CPU运行指令的原子性，即使是异步的中断也不能打断一条指令的执行，故中断处理程序在中断发生后，执行完当前指令后执行，并返回到当前指令的下一条指令）==
2. 软件中断-**陷阱与系统调用**：在现代操作系统下运行的程序往往被严格限制了操作的权限，例如读写文件、发起网络请求这类功能，需要操作系统代为执行。**陷阱与系统调用**便是操作系统提供服务（SDK）的接口（API）的产物。程序可通过使用系统调用函数来向操作系统发起请求，处理器执行系统调用的处理程序，再回到原程序。
3. 软件中断-**故障**：处理器运行过程会遇到一些错误，这在之前有所涉及。一般**故障**指故障处理程序有可能能够处理的异常，如"请求的虚拟内存中的内容还未加载到内存上"。
4. 软件中断-**终止**：**终止**指严重的错误，异常处理程序将结束原程序。

在CPU中的特殊异常表基址寄存器中存储了异常表，用于按照异常代号进行跳转。

中断处理程序特殊之处：

+ 运行于内核模式下，拥有完全访问权限
+ 中断的处理不涉及用户上下文，不使用进程内核资源（中断拥有自己特有的资源）
+ 不维护task_struct等复杂结构，不可被调度，仅能被更高级的中断（名词）中断（动词）
...
1. 寄存器上下文：通用寄存器、程序寄存器、处理器状态寄存器、栈指针
2. 用户级上下文：数据段、用户堆栈、共享存储区
3. 进程级上下文：进程控制块task_struct、内存管理信息（mm_struct、vm_area_struct、pgd、pte）、内核栈
...
内核对执行进程的选择，称为**进程调度**，由内核代码**调度器**负责。切换执行的进程，称为对原进程的**抢占**，新进程的**调用**，这一过程包括：

1. 切换至内核态
2. 寄存器上下文（用户态信息）压入内核栈（硬件指令自动完成）
3. 保存寄存器上下文（内核态信息）
4. 导入下一进程的寄存器上下文（内核态）
5. 切换至下一进程内核态
6. 恢复下一进程寄存器上下文（用户态）
...
上下文切换中，模式发生：用户-内核-用户的切换

需要尤为注意的是，普通进程在运行过程中可能受到**调度**而停止运行，但是这一点对于前述**硬中断**来说需要额外考量：

1. 非硬中断异常：与通常进程无异，即执行内核段中的代码，可能被**进程调度**
2. 硬中断异常：相当特殊，在进行中断处理函数之前，一般关闭进程调度，因为中断处理一般不维护数据结构，且数量多、速度快，故设计为不能被打断，除非是更高级的硬中断。

PS：内核，是操作系统中的关键程序集合，用于处理进程管理与各种系统调用。其实现方式较多，早先内核存在独立进程；现今，则寄存在进程中。

###### 7.2.1.5.2 系统调用

系统调用前后执行的仍旧是同一个进程，其上下文切换较为简单，包括：
...
注意到该过程不涉及保存寄存器上下文（内核态），可能与内核态每次开始状态相同有关……（仅猜测）
...
线程为轻量级进程，同一个进程的线程共享进程级上下文，故进程间切换：
...
对于CPU而言，中断的优先级总是大于进程的，故在遇到中断时：

1. 关闭进程调度
2. 寄存器上下文（用户态）压入内核栈
3. 恢复寄存器上下文（内核态）
4. 切换至内核模式
5. 使用**中断栈**（CPU中特殊的硬件结构）保存寄存器上下文（内核态）
6. 硬中断处理
7. 恢复寄存器上下文（内核态）
8. 恢复寄存器上下文（用户态）
9. 切换至用户模式
...
硬中断可能发生在任何时间、任何地点，运行于**中断上下文**（包括特殊的中断栈）

此处对系统调用与硬中断作出区分：

系统调用属于软中断，并非硬中断

二者机制基本相同，但是硬中断一般执行较快，且频繁，故关闭进程调度。

PS：系统调用错误处理：系统调用接口函数在调用发生错误时，将显式返回信号。程序应当显式对其进行处理。可以通过**错误处理包装函数**来一体化调用与错误处理，更为简洁。
...
1. 运行：正在执行或等待运行
2. 停止：在信号的控制下暂时停止执行，等待信号指示
3. 终止：由于1）主程序返回 2）收到终止信号 3）调用 exit函数

操作系统启动时，总是先通过特殊的方式开启第一个进程：PID=1，其余进程从其中生成。

使用exit结束进程：
![](./assets/Picture/7/exit.jpg)

使用fork函数生成子进程：
![](./assets/Picture/7/fork.jpg)

生成的子进程与父进程拥有近乎完全相同的属性（除了与进程调度相关的资源PID等等）。

exit函数从不返回，而fork函数则调用一次返回两次。

父进程除了生成子进程外，还拥有回收子进程的义务。

进程终止后，其大部分资源被释放，但是，其PID及部分终止相关信息却被保留，此时，称为**僵死进程**，直到其父进程调用waitpid函数对齐进行回收。

![](./assets/Picture/7/waitpid.jpg)

该函数可以通过设定pid、statussp、options项来调整回收对象、回收状态的检测以及自身的阻塞与否等属性。

父进程先于子进程终止时，子进程成为**孤儿进程**，由init进程（PID=1）进行管理。

休眠与挂起：使用sleep与pause使进程**停止**一段时间或直到收到信号。

![](./assets/Picture/7/sleep.jpg)
![](./assets/Picture/7/pause.jpg)

使用execve使用当前进程执行新的程序：

![](./assets/Picture/7/exec.jpg)

并且除非装载失败，将不再返回。

**进程装载**:

![](./assets/Picture/7/argc_env.jpg)

![](./assets/Picture/7/stack.jpg)

execve函数将初始化进程栈帧如上，其将重新安排虚拟地址空间，但是保留PID、文件描述符与信号上下文。

### 7.3 信号

前文多次提及**信号**，信号是一种更为高级的异常控制流表现形式。
作为一种软件形式的异常控制，信号提供了进程间通讯的手段，使得多进程程序成为可能。

以下为Linux系统的30种信号

![](./assets/Picture/7/Signal.jpg)

#### 7.3.1 信号的发送

内核通过修改进程上下文的某个状态位，来向其传递信号。一般见于：

1. 内核检测到系统事件
2. 当前进程使用kill函数显式发送信号

Unix系统下，进程归属于进程组，子进程默认继承父进程的进程组。

可以使用getpgrp与setpgrp来获取与设置进程的进程组ID。

![](./assets/Picture/7/get.jpg)
![](./assets/Picture/7/setgroup.jpg)

值得注意的是，setpgrp中参数为0时表示使用调用该函数的进程的pid与进程组ID,使用不存在的进程组ID将直接创建之。

我们可以在终端使用/bin/kill -n -pid 指令向指定pid发送指定n信号。

命令行中的程序分为**前台**与**后台**，前台程序往往阻塞，后台则非阻塞。一般每个程序的执行将产生一组进程，组成进程组。我们可以使用Ctrl+C发送SIGINT信号给前台进程组，使用Ctrl+Z发送SIGTSTP信号给前台进程组（使用fg指令唤醒）。

我们还可以通过alarm函数设置时钟，在一定时间后给自己发送SIGALRM信号。该函数并在已有时钟存在的情况下结束前一个时钟，并返回剩余秒数。

#### 7.3.2 信号的接收

当目的进程对信号做出反应，就称为接收信号。信号的接收形式包括：

1. 忽略
2. 终止
3. 终止并转储内存
4. 停止（挂起）直到被SIGCONT信号重启
5. 执行**用户层函数-信号处理函数**

每个进程的待处理信号空间中包括30个位，置高表示有这类事件待处理。但是无法存储超过一个同类信号。

进程从内核态转回用户态时，会检测未阻塞的待处理信号集合，选取一个进行处理。

信号默认行为为1-4，但我们可以通过signal函数改变其行为为某个处理函数。

![](./assets/Picture/7/signalfunc.jpg)

其中handler可以取值SIG_ING 用以忽略信号；取值SIG_DFL，用以恢复默认行为。
但是，SIGSTOP与SIGKILL信号的行为是不允许修改的。

信号处理同异常相似，但是**异常处理子程序是内核共享代码；而信号处理子程序则是进程独享的用户级代码**。

PS：中断与信号关联：

1. 系统调用可能发生信号处理
2. 硬中断不会被打断，不存在信号处理，反而信号处理时可能发生中断

进程也可以暂时不对信号作出反应，但又不是忽略，也许是一段时间后再作处理。此时就用上**信号阻塞机制**。

![](./assets/Picture/7/blocked.jpg)

how取值SIG_BLOCK/SIG_UNBLOCK/SIG_SETMASK来将阻塞集合加上、减去、设为set。

还需要注意到，信号处理程序总是阻塞其所处理的同款信号。
...
1. 安全的：可重入（仅访问局域变量等）或不可打断
2. 正确的：能够正确处理信号
3. 可移植：能够在不同的系统上运行，可以使用函数来重定义信号处理行为

另外，还当认识到，信号处理子程序虽然与主程序是互相独立的逻辑流，但是却运行在同一个进程当中。

#### 7.3.4 同步流

操作系统对于各个进程的执行顺序是不受我们管理的，所以涉及多个进程交互的信号处理程序应当考虑到进程之间发生的**竞争**。

#### 7.3.5 显示等待信号

有些程序需要显示等待信号才能继续执行，按照已有的知识体系，解决方案如下：

略

### 7.4 非本地跳转

非本地跳转亦是一种高级异常控制流的表现形式。

解释非本地跳转之前，需要对函数调用进行一般的说明。

一般而言，函数之间互相调用，其栈帧层层累积，若是想要回到最初的函数，需要一层一层地进行返回，而且每次返回都将出现在调用后的位置上，处理较为繁琐（需要对每个这个调用连锁中的每一个函数设置特殊的跳转条件指令）。

而非本地跳转提供了一种快速跳出的途径，而且可以指定代码位置，不受返回时的位置影响。

非本地跳转由一对函数实现：**setjump** 与 **longjump**

![](./assets/Picture/7/setjump.jpg)

![](./assets/Picture/7/longjump.jpg)

其中，setjump函数被调用时，将当前的**调用环境**（PC、栈指针、通用寄存器）存储到env缓冲区中。

而longjump函数调用时，则恢复最近一次调用的setjump所保存的**调用环境**，暴力地恢复了程序地状态。由于该恢复不包括堆与内存，程序在先前的执行中进行的内存处理不会被逆转，且堆中申请的内存也不会被释放，这需要注意。
...
+ 未分配：不对应磁盘或内存，不占空间
+ 缓存的：不仅对应磁盘，还对应物理内存的物理页（由物理页号标识）
+ 未缓存的：仅对应磁盘上的数据块（由文件描述符、数据偏移标识）

##### 8.1.3 主存缓存机制

作为不命中惩罚巨大的高速缓冲机制，DRAM是缓存是**全相联**的：任何的虚拟页可以缓存至任意物理页中。

DRAM缓存机制的实现依赖与操作系统、MMU以及物理内存中的**页表**（PT）。

![](./assets/Picture/8/MP.jpg)

**页表**（PT）即为**页表条目**（PTE）的数组，每个页表条目由**有效位**与**地址**组成。
...
牺牲页的页表条目的地址将改换为磁盘中的地址（或可执行文件中的不可变数据（代码段等），或来自匿名页的运行时数据对象），有效位设为无效。
...
![](./assets/Picture/8/Symbols.jpg)

![](./assets/Picture/8/Translation.jpg)

地址翻译：

1. 取虚拟地址VPN，作为PTE表（页表）索引
2. 用索引取得对应PTE，判断有效性
3. 分情况讨论:
   + 无效，引发异常
     + 缺页异常处理程序
...
![](./assets/Picture/8/Hit_and_Miss.jpg)

一些细节问题：

1. Q：高速缓存归为虚拟寻址还是物理寻址？
   A：使用物理寻址，无需考虑地址转换
2. Q：翻译后备缓冲器（TLB）？
   A：页表的高速缓冲，高相联度，加速地址转换
3. Q：多级页表？
   A：采用树的结构管理页表，节省空间

整个管理过程由称为**页式内存管理**的机制来完成

##### 8.1.6 Intel CPU对地址的管理

**逻辑地址**，即机器码中指令地址。

Intel CPU使用CPU内部结构实现**段式内存管理**进行将机器指令中的地址转换为**线性地址/虚拟地址**。过程中使用 **段基址寄存器**、**段变址寄存器**、**偏移寄存器** 进行计算，实现转换。

举例：相对寻址模式下：call x，那么逻辑地址即为x+eip，而线性地址由于归0，故等于逻辑地址。此后再进行页式内存管理。在页式管理机制下，每个段其实都被分页，而线性地址也就成了虚拟地址（毕竟一开始就是在一个假想空间中进行的分段），再通过页表寻址转换为物理地址。

来由：在16位的8086cpu中，可以访问的地址为2^16^=64kb，无法利用更大的内存。于是采用分段机制，将内存分为多个段，每个段由一个16位寄存器管理（管理多个段），CS是代码段，DS是数据段，SS是堆栈段，ES是附加段。进程的文件也按照类型分配到一定的段上，使用时仅需给出段地址（16位）与段偏移（4位），即可访问到所需的内存空间。**这是一种最初的内存分离管理机制**（分段）

<https://www.cnblogs.com/johnnyflute/p/3564894.html>
<https://www.zhihu.com/question/50796850/answer/1717457022>

发展：

1.开始的分段机制中，内存管理为实地址模式，段寄存器是普通寄存器，可由程序随意设置，一个程序可以随意访问任意内存空间；后来的80386中推出保护模式，将段分离开，程序按段描述符进行访问，在GDT中查询段的大小、保护信息、权限信息与段基地址，再加上段偏移，在经过安全检测后再进行访问，同时将设置段寄存器的指令设置为特权指令，避免任意修改，就实现了基于分段的保护模式。

1. 随着分页内存管理机制的出现，段式内存管理与页式内存管理融合在一起，发展出了**段页式内存管理**：即先分段，再分页。首先将内存照旧分段，再将映射到一个段中的内容分为多个页，之后按照分页管理机制进行管理。此时，线性地址为虚拟地址。

这个机制为intel的CISC独有，故考虑到可移植性，一般直接设基址寄存器为0，将段大小设为最大，即整个内存为一个段，全部页都存在这一个段中。

PS：可执行文件中段（section）、CPU段寄存器及虚拟内存段（segment）的关系：

1. 在分页模式未出现之前，操作系统按照文件中的段来设置段寄存器基址，分配空间，依赖分段管理来实现**虚拟地址**（与页式管理无关）。但是这样分配粒度太大（整段同时存入内存），无法充分发挥**局部性**。此时，文件中段是与CPU中段进行绑定的，方便管理。

2. 在分页模式出现后，便可以将一个段分为多个页，按照页表进行映射缓存，极大发挥了物理内存的缓冲作用。（只需要将段中被调用的页缓存即可）。同时，页式管理提供了更加丰富的权限管理（vm_area中的描述信息及页表附加项），段式管理就显得多余了，于是不再分段。所有的可执行程序中的段，也就被映射到了一个**段**（CPU中的段）的不同位置上，由页进行区分。**每个程序段至少占着自己的页**，此时，程序段与CPU段就脱勾了。

3. 由于虚拟内存是不关心每个section中的内容去别的，其所关注的仅仅是其读写执行权限而已，故链接过程中，会将文件中权限一致的section放在一起，形成segment，而内存映射时便是按照segment进行映射的。所以.init与.text section便映射到同一个segment区域中了。按照权限，至少存在3各segment：
   1. 可读可执行：.init .text等等
   2. 可读可写：数据段与bss段
   3. 只读：只读数据
...
### 8.2 虚拟内存机制的其余组件和高级应用

虚拟内存机制与程序的运行息息相关，为更好地了解虚拟内存在进程中地高级应用，我们需要先介绍Linux的进程内存结构：

![](./assets/Picture/8/mm.jpg)

内核为每个进程维护其独有的task_struct结构，其中包含该进程相关的许多信息，其中，mm_struct结构与与虚拟内存高度相关。

mm_struct 包括：

1. pgd：指向第一级页表地址（一级页表常驻内存，进程调度到时转入CR3寄存器？）
2. mmap：指针，指向vm_area_struct链表，每个vm_area_struct包含某一段的开始、结束地址及访问许可信息

可以说vm_area_struct记录了**虚拟内存段**的相关信息。

触发缺页异常时，将用到vm_area_struct中的信息，判断访问的合法性与安全性。
（**页表条目与vm_area_struct的双重保护机制？** 答：页表条目区分代码与内存；vm区分内核、用户权限）

通过这些数据结构与异常，Linux可以实现虚拟内存系统的全部功能。

#### 8.2.1 内存映射

内存映射：将磁盘文件对象与虚拟内存区域关联起来的机制，该机制通过更新页表与vm_area-struct链表，实现磁盘文件或物理内存中的匿名文件在虚拟内存中的映射。

经过内存映射，操作系统完成了对进程中mm_struct中页表、vm_area_struct结构的初始化。此时，物理内存当中仅有部分缓存（匿名文件），但是页表中已有相应磁盘索引，可以转交给前述缺页机制进行处理。

#### 文件类型

可以被映射的文件有两种：

1. **普通可执行文件**：存在于磁盘上的普通文件，归属文件系统管理，用于映射代码段、只读数据段，其初始化使用文件内容，余下用0补齐。
2. **匿名文件**：只存在于内存中文件，可以用于映射堆、栈中的运行时对象，其初始化请求二进制0.

##### 交换文件

**swap file/pagefile**：这是一类较为特异的磁盘空间，用于储存由于页面调度被替换出来的页。其大小限制着进程能够分配的虚拟页的上限数量。
值得注意的是，从内存中被替换出来的页面不总是存入交换分区，比如文件，就可以直接释放回文件系统；而栈、堆页，就必须放入交换页面了。

初始化后的页面在内存及交换文件间交换。

##### 共享对象

虚拟内存的映射机制，使不同进程的共享对象映射到物理内存上的同一块地址区域。

##### fork

fork函数会复制原进程的task_struct的部分数据、页表内容等，使二者的私有空间映射至同一物理地址。
使用**写时复制**，直到其中一方进行修改操作，才会进行复制，提高内存效率与fork函数的效率。

**写时复制**由**保护-故障**中断实现。

##### exec

exec函数在当前进程执行新的程序。重置包括task_struct在内的一系列数据，加载分配虚拟内存空间。
...
以上的内存映射都是由操作系统代为完成的，实际上也可以使用mmap函数手动操作。

![](./assets/Picture/8/mmap.jpg)

mmap函数将请求内核在start处开始，分配length长的内存空间，使用prot设置权限，flags设置类型，若为普通文件类型则从fd的文件描述符处的文件的offset偏移处开始映射。（不一定从start处开始）

也可以使用munmap删除虚拟内存区域。

![](./assets/Picture/8/munmap.jpg)

#### 8.2.2 显示动态内存分配（malloc）/隐式动态内存分配（垃圾收集器）

出于处理未知大小数据的需要，有时我们需要手动分配内存空间。

分配器有两种风格，主要区别于其对内存的回收方式：

1. 显式（malloc&free）：手动分配与回收
2. 隐式（垃圾收集器）：由垃圾收集器回收内存

动态内存分配发生在内存的许多层面，包括显存、主存，这里主要分析**堆**的内存分配。

##### 8.2.2.1 malloc&free

![](./assets/Picture/8/malloc.jpg)
![](./assets/Picture/8/free.jpg)
![](./assets/Picture/8/sbrk.jpg)

一般使用malloc分配堆内内存空间，free对从malloc返回的指针进行释放（不能对其余指针或发生偏移的malloc指针进行free，行为未定义）

二者的实现依赖于srbk函数。该函数可以通过设置堆顶指针，收缩或增加堆的空间。

[!_!] ==后续将引入红黑树实现的动态内存分配器==
...
虚拟文件系统，通过在磁盘文件与物理内存间插入中间层来更好地协调进程的数据管理：

1. 磁盘文件由文件系统管理，进程通过**文件描述符**进行调用
2. 虚拟内存系统按**页**管理数据，由pdg指向的页表（记录访问许可信息与地址）与mm_struct中的vm_area_struct（记录所属段与访问信息）对虚拟页进行管理。
3. 物理内存系统由多级缓冲机制进行管理，其中**页面调度**、**替换策略**如**缓存机制专题**所示
4. 内存映射：将1、2联系在一起。通过文件系统读取磁盘文件内容，按照可执行文件的**段**将文件内容分类，再分页。其后，**抽象来说**：按照文件所属映射信息映射到虚拟内存（即安排空间）；**实际实现**：将访问信息、映射地址则记录到vm_area_struct中，再更新pdg指向的页表中的条目。
5. 虚拟访问机制：将2、3联系在一起（也可以说是1、2、3），实现物理内存作为磁盘的缓存。具体落实为，通过查询页表进行地址翻译，将虚拟地址转化为物理地址再访存；缺页则通过缺页中断加载页到物理内存中，该过程中的访问合法性与安全性由页表条目的附加项与vm_area_struct中的信息保证。
...
Linux系统使用三个相关的数据结构来管理文件：

1. **描述符表**（descriptor table）：进程独立拥有，管理进程相关的描述符。
初始默认0、1、2指向标准输入、标准输出、错误输出。每个描述符指向一个**文件表**。
2. **文件表**（file table）：为操作系统统一，所有进程共享。记载文件的位置、引用计数及v-node表指针。用于管理整个系统对文件的访问。
3. **v-node表**：文件系统生成，所有进程共享。在系统的文件管理系统中，对其下属全部文件的信息进行统一管理，供给进程读取调用。

### 9.3 重定向

使用dup2函数改变描述符表中某一描述符所指向的文件表，即改变以该描述符所表示的输入输出方向。

### 9.4 流的使用与一些细节

1. 标准IO：标准IO将文件抽象为流，所有操作都在流上进行，较为方便。使用前需要打开，使用后则需要关闭。
   但是标准IO允许同时在一个流上进行读写，全双工，但是却又存在诸多限制，影响了套接字（网络）操作。
2. RIO函数：RIO函数具有较强的健壮性，适用于网络操作。
3. UNIX IO：最为底层的程序，不提供缓冲，读写时容易引起极多的系统调用，造成效率较低下。

---

之后的内容，课堂省略，个人也由于缺乏自驱，放弃记录。
